...
local assert, pcall, xpcall, error, setmetatable, tostring, tonumber, type, pairs, ipairs =
    assert,
    pcall,
    xpcall,
    error,
...
do
    ...
    SteamAPI_RegisterCallResult =
        find_sig(
        "steam_api.dll",
        "\x55\x8B\xEC\x83\x3D\xCC\xCC\xCC\xCC\xCC\x7E\x0D\x68\xCC\xCC\xCC\xCC\xFF\x15\xCC\xCC\xCC\xCC\x5D\xC3\xFF\x75\x10",
        "void(__cdecl*)(struct SteamAPI_callback_base *, uint64_t)"
    )
    SteamAPI_UnregisterCallResult =
        find_sig("steam_api.dll", "\x55\x8B\xEC\xFF\x75\x10\xFF\x75\x0C", "void(__cdecl*)(struct SteamAPI_callback_base *, uint64_t)")

    SteamAPI_RegisterCallback =
        find_sig(
        "steam_api.dll",
        "\x55\x8B\xEC\x83\x3D\xCC\xCC\xCC\xCC\xCC\x7E\x0D\x68\xCC\xCC\xCC\xCC\xFF\x15\xCC\xCC\xCC\xCC\x5D\xC3\xC7\x05",
    ...
    steam_client_context = find_sig("client.dll", "\xB9\xCC\xCC\xCC\xCC\xE8\xCC\xCC\xCC\xCC\x83\x3D\xCC\xCC\xCC\xCC\xCC\x0F\x84", "uintptr_t", 1, 1)
    ...
    _G._http = {
        ...
        call_result_cancel = call_result_cancel,
        callback_base_ptr = callback_base_ptr,
    ...
    callbacks.Register(
        'Unload',
        function()
            for key, value in pairs(_G._http.pending_call_results) do
                local instance = ffi.cast(_G._http.callback_base_ptr, value)
                _G._http.call_result_cancel(instance)
            end

            for key, value in pairs(_G._http.registered_callbacks) do
                local instance = ffi.cast(_G._http.callback_base_ptr, value)
                _G._http.SteamAPI_UnregisterCallback(instance)
            end
            _G._http = nil
        end
    )
...
local HTTPRequestCompleted_t_ptr =
    typeof(
    [[
struct {
...
local HTTPRequestDataReceived_t_ptr =
    typeof([[
struct {
	http_HTTPRequestHandle m_hRequest;
	uint64_t m_ulContextValue;
