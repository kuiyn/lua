...
-- ported to aimware lua api by qi
-- Includes LZW decompression.

local ffi = require "ffi"
local bit = require "bit"

local draw_CreateTexture, string_format, table_insert, bit_band, bit_rshift, bit_lshift, ffi_new, ffi_copy, ffi_fill, ffi_string =
    draw.CreateTexture,
    string.format,
    table.insert,
    bit.band,
...
local function gifdecode(self)
    ...
    while true do
        ...
        elseif blocktype == GIF_IMG_BLOCK then
            ...
            local texture = draw_CreateTexture(data_str, iw, ih)
...
local function gifdrawframe(self, frame, x, y, w, h, r, g, b, a)
    if self.frames[frame] == nil then
        error("Frame not found", 2)
    end

    draw.SetTexture(gifframe(self, frame))
    if r and g and b then
        draw.Color(r, g, b, a or 255)
    end
    draw.FilledRect(x, y, x + w or self.width, y + h or self.height)
    draw.SetTexture(nil)
end
