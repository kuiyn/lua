...
local native_read_file = vtable.bind(filesystem, 0, "int(__thiscall*)(void*, void*, int, void*)")
local native_open_file = vtable.bind(filesystem, 2, "void*(__thiscall*)(void*, const char*, const char*, const char*)")
local native_close_file = vtable.bind(filesystem, 3, "void(__thiscall*)(void*, void*)")
local native_file_size = vtable.bind(filesystem, 7, "unsigned int(__thiscall*)(void*, void*)")

local function engine_read_file(filename)
    local fp = native_open_file(filename, "r", "MOD")
    if fp == nil then
        return
    end
    local size = native_file_size(fp)
    local buf = c_char(size + 1)
    native_read_file(buf, size, fp)
    native_close_file(fp)
...
local steam_api_ctx =
    cast(
    "S_steamApiCtx_t**",
    cast("char*", client.find_signature("client.dll", "\xFF\x15\xCC\xCC\xCC\xCC\xB9\xCC\xCC\xCC\xCC\xE8\xCC\xCC\xCC\xCC\x6A")) + 7
)[0]

local get_friend_avatar = {}
local avatar_type = {"-", "m", "+"}
local steam_friends = cast("void***", steam_api_ctx.steam_friends)
for i = 1, 3 do
    get_friend_avatar[avatar_type[i]] = vtable.bind(steam_friends, 33 + i, "int(__thiscall*)(void*, uint64_t)")
end

local steam_utils = cast("void***", steam_api_ctx.steam_utils)
local get_image_size = vtable.bind(steam_utils, 5, "bool(__thiscall*)(void*, int, uint32_t*, uint32_t*)")
local get_image_rgba = vtable.bind(steam_utils, 6, "bool(__thiscall*)(void*, int, unsigned char*, int)")
...
local function get_steam_avatar(steamid3_or_steamid64, scale)
    local t = scale or "m"
    local cache_key = string.format("%s_%s", steamid3_or_steamid64, t)

    if steam_avatars[cache_key] == nil then
        local func = get_friend_avatar[t]
