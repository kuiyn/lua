...
local wapon_system =
    ffi.cast("void****", ffi.cast("char*", client.find_signature("client.dll", "\x8B\x35\xCC\xCC\xCC\xCC\xFF\x10\x0F\xB7\xC0")) + 0x2)[0]
local get_weapon_info = vtable.bind(wapon_system, 2, CCSWeaponInfo_t .. "*(__thiscall*)(void*, unsigned int)")
...
for idx = 1, 1000 do
    ...
    if res ~= nil then
        ...
        weapon.name = localize(weapon.item_name)
        weapon.raw = res
